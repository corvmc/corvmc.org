...
export const getReservationEndTimes = query(
	z.object({
		date: z.string(),
		startTime: z.string(),
		// Who is booking, because the duration floor is a fact about them. Without
		// it a teaching booking is only ever offered hour-long ends: the server
		// accepts a half-hour lesson and the picker never lets anyone ask for one,
		// which is the whole point of the teaching minimum.
		bookerType: z.enum(bookerTypes).optional()
	}),
	async ({ date: dateParam, startTime, bookerType }) => {
		const dateStr = dateParam || formatDateInTz(new Date(), DEFAULT_TIMEZONE);
		const [slots, reservationConfig] = await Promise.all([
			getAvailableSlots(dateStr),
			getReservationConfig()
		...
		const { minDurationHours } = termsFor(bookerType ?? 'user', reservationConfig);
		const minSlots = minDurationHours * (60 / slotMinutes);
