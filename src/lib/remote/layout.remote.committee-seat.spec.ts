...
/**
 * A committee seat that grants a staff module (#1578) has to open the staff
 * panel on its own, or a member holding no position has the guard's consent
 * and no page to use it on.
 */
...
describe('getStaffLayout', () => {
	it('opens the panel to a Development committee member holding no position', async () => {
...
describe('getMemberLayout', () => {
	it('offers the Staff panel tab to a committee seat that grants a staff module', async () => {
		committeeCapabilitiesFor.mockResolvedValue(SEAT);
