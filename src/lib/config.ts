...
/** `pending` is a guest pledge whose email is unconfirmed; it holds nothing. */
export const wishlistPledgeStatuses = ['pending', 'open', 'fulfilled', 'released'] as const;
 ...
 * capability no position grants.
 *
 * `audit.read` guards only the cross-member log; one member's History card is
 * part of their record and stays on `user.read`.
 */
