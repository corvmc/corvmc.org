...
export function createStripeGateway(): PaymentGateway {
	if (!_stripe) {
		...
		/**
		 * Pinned to the version this SDK's types were generated from. Unpinned, the
		 * client follows the account's default, which Stripe rolls forward on its
		 * own schedule — the request version would move without a commit while the
		 * types did not. The literal is the SDK's single-valued `ApiVersion`, so
