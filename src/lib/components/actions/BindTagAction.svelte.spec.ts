 ...
 * modal title and the panel above the field all flip on `currentTag`. That
 * matters because a rebind is a *different* claim — the unit keeps its id,
 * its loans and its repairs, and only the sticker changes — and the dialog is
 * the only place that is said. What a scan *means* is `parseScan`, covered by
 * `src/lib/utils/scan.spec.ts`.
 */
