...
type Usage = { file: string; line: number; attr: string };

function usagesOf(component: string, files: string[]): Usage[] {
	const found: Usage[] = [];
	for (const file of files) {
		...
		const visit = (node: unknown): void => {
			...
			if (n.type === 'Component' && n.name === component) {
...
describe('Action call sites', () => {
	it('pass no prop Action does not declare', () => {
		const declared = declaredProps(path.join(src, 'lib/components/ui/Action.svelte'));
		const stray = usagesOf('Action', svelteFiles(src)).filter(
			(u) => !declared.has(u.attr) && !HTML_PASSTHROUGH.test(u.attr)
		);
	...
	it('can gate the submit through canSubmit', () => {
		expect(declaredProps(path.join(src, 'lib/components/ui/Action.svelte'))).toContain('canSubmit');
	});
});
