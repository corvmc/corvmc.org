---
title: Handle the Inbox
slug: staff-inbox
category: staff-guide
summary: Read and respond to contact-form and channel messages.
minRole: staff
sortOrder: 14
---
...
**Open** is what needs a human right now. **Snoozed** is everything parked —
conversations you put on a date, and ones you have answered that are waiting on a
reply. Both come back on their own, which is why they share a tab; each row still
says which of the two it is, and a dated one says when it is due back.

Below that you can **search** by sender or subject, and filter by **channel**, by
**status**, by **inquiry type**, by who a conversation is **assigned to**
(including _Unassigned_ and _Assigned to me_), and by how long it has been
**waiting**. Your filters live in the address bar, so opening a conversation and
...
A conversation marked as awaiting a reply moves out of Open and into Snoozed, and
stops counting towards the number on the Inbox nav item — that number is there to
say how much is waiting on _you_. Anything the contact sends clears the mark on
its own, whatever channel it arrives on, and the conversation is back in Open.

## Tips

- Resolve threads once handled so the Open queue stays honest.
- A conversation waiting on a reply comes back to Open after a week if nobody
  writes, so a contact who goes quiet cannot take it out of the queue for good.
...
- The public [contact form](/member/help/staff-dashboard) feeds this inbox.
