<script lang="ts">
	 ...
	 * `/staff/events` — that page is scoped by source and runs the shows CMC
	 * produces. A CMC show is on both, in two roles: something you are building,
	 * and something the public can see.
	 *
	...
	$effect(() => {
		...
		const href = `${resolve('/staff/calendar')}${search ? `?${search}` : ''}`;
