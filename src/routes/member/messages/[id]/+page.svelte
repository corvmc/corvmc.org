<script lang="ts">
	/**
	 * One conversation, of four kinds since #1250: a DM, a staff thread, a
	 * group's chat and a band's booking enquiry. The last two are panes of
	 * their own — different reply identity — so this file dispatches.
	 *
	 * Within a DM the order matters: a pending request shows Accept / Decline /
	 * Report *instead of* a box, which keeps a request to one message.
	 */
...
<ThreadPane>
	...
	{:else if t.kind === 'enquiry'}
		...
		<EnquiryThread thread={t} backHref="/member/messages" onchanged={refreshConversations} />
	{:else}
		<ThreadHeader
			{title}
