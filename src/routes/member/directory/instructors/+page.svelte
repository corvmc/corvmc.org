<script lang="ts">
	...
	import { getMemberInstructors } from '$lib/remote/instructors.remote';
	...
	const instructors = $derived(getMemberInstructors({ search: searchText || undefined }));
</script>
...
<PageContent>
	<div class="mb-6 flex justify-center">
		<TabBar
			tabs={[
				{ key: 'bands', label: 'Bands', href: resolve('/member/directory') },
				{
					key: 'musicians',
					label: 'Musicians',
			...
			{:else}
				<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{#each rows as row (row.userId)}
						<InstructorCard
							href="/member/directory/members/{row.userId}"
							name={row.name}
							image={row.image}
