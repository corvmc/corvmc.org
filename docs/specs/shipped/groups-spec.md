...
Soft-deleting a document **detaches it and lets the sweep reclaim the object**; the row is the audit record until then. The original rule here was an immediate hard-delete, reasoning that a soft-delete flag with no reaper is how storage bills grow silently. That reasoning was right and held only while no reaper existed — [`media-spec.md`](shipped/media-spec.md) has since built one (`/api/cron/sweep-media`), so the case for deleting inline is gone and the case against it applies here as it does everywhere else: a delete that fires from a write path cannot see whether anything else still references the object, and if the R2 call fails after the row is gone the file is stranded with no record of its key.
...
`requireBandMember` / `requireBandAdmin` / `requireBandOwner` became thin deprecated wrappers
delegating to `requireGroupRole` in #323, so the guard and the 41-call-site port landed in separate
reviewable PRs; #324 ported the call sites and deleted `band-context.ts`.

Which ref a call site passes is per-file rather than uniform, and deliberately so. `bands.remote.ts`
names its band by **id**, because three of its queries already took a `bandId` and cross-checked it
against the band the old guard resolved from `params` — handing the id straight to the guard deletes
that seam rather than translating it. Everything else passes the `slug` it already had.
...
**Phase 8 is deferred and 9 and 10 run ahead of it** (decided 2026-08-30). Documents is the one phase
whose first step is infrastructure rather than code — a second R2 bucket has to be deployed and
verified before its table is worth writing — and nothing in 9 or 10 depends on it. The order within
the rest still holds: 9 renames `event.bandId` and 10 re-keys `event_band`, both in the same schema
file, so running 10 first would mean resolving that collision twice.

| #   | State                       | Phase                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | ✅ #247                     | Reserved slugs. First, and near-irreversible once groups exist                                                                                                                                                                                                                                                                                                                                                                                 |
| 1   | ✅ #267                     | `band` → `group`: the row keeps its identity and the table is renamed (see below), gaining `kind`, `joinPolicy` and `joinInstructions`. `band_member` is untouched — its foreign key simply points at the renamed table. Rename the `'band'` booker type to `'group'`; no `bookerId` repoint is needed                                                                                                                                         |
| 2   | ✅ #279                     | `band_member` → `group_member`, with every read and write ported and the CI grep gate — **its own PR**                                                                                                                                                                                                                                                                                                                                         |
| 3a  | ✅ #283 #285 #287 #290      | `directory_entry` + `directory_tag`: create, backfill one entry per band **and** per member, fold in `band_genre`/`user_genre`/`user_instrument`, migrate readers. Columns stay on `user`/`group` until 3c. **Four PRs**: schema + backfill, then the band half, then the member half, then the merges and the gate — split by surface, because reads and writes for one surface cannot move apart without a window where an edit is invisible |
| 3b  | ✅ #292 #303                | `band_site`: move tier, subscription and the five `customDomain*` columns; re-key `band_page_config` and `band_media`. Carries the `band-host-service.ts` join                                                                                                                                                                                                                                                                                 |
...
| 8   | ✅ #431 #434                | Documents — bucket and binding deployed and verified **first** (#420), then the table and route. **Clubs and committees only**; bands were dropped — see below                                                                                                                                                                                                                                                                                 |
...
> **Re-verified 2026-09-13 (#1103).** Three of the eight rows are struck: the `processEventSeries`
> hard-coding, the raw `band_member` subqueries, and the owner inner joins. The remaining five were
> not re-checked in that pass, so read a live row as "unverified since the spec was written" rather
> than as current. What #1103 did check end to end is the Real Book Club workflow this spec names as
> its driving case — every step of which now has a path.

| Finding                                                                                                                                                                                  | Location                                                                  | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~`processEventSeries()` hard-codes `source: 'cmc'` and `status: 'draft'`, copying neither owner nor `location`~~ — **fixed in #338**                                                    | `generation-job.ts` — `processEventSeries`                                | A club's recurring jazz night would never reach the gig guide **and** would not hold the room. An occurrence now inherits `source`, `groupId` and `location`, publishes when `source !== 'cmc'`, and maintains the `event_group` and `event_band` invariants that copying `groupId` makes it owe.                                                                                                                                                                                                                                                                                                        |
...
| ~~Three raw-SQL `band_member` subqueries~~ — **resolved: no raw `band_member` SQL survives in service code**                                                                             | `band-service.ts` — inside `listForUser`, `listAll`, `getByIdWithDetails` | Invisible to `pnpm check`; throw at runtime after the table is dropped. Needs a CI grep gate on the literal string as part of phase 2.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ~~Three `innerJoin(user, eq(user.id, band.ownerId))` — twice in `listAll`, once in `getByIdWithDetails`~~ — **resolved: `ownerId` was dropped in phase 3c, so there is no join to make** | `band-service.ts`                                                         | An ownerless group is legal under this spec, and an inner join drops any row whose owner is missing. In `listAll` that hides precisely the groups staff need to act on; in `getByIdWithDetails` it empties the detail page of a group that still exists. All three must become left joins during the port. See [Ownership](#ownership).                                                                                                                                                                                                                                                                  |
...
**A decision this spec must make, not defer:** whether `processEventSeries()` copies `status` from the prototype. Doing so is required for a club series to publish automatically, but it changes behavior for existing staff CMC series, which today always generate drafts for review. Publish automatically only when `source !== 'cmc'`, preserving the staff review step where it already exists. **Shipped that way in #338.**
