...
Three flags: `groups`, `groupEvents`, `announcements`. The last two cover bands as well as groups, since both capabilities key off group membership and bands are groups. A fourth, `groupFiles`, was registered for phase 8 and retired unused when phase 8 was deferred — Documents gets a feature branch when it is built, not a flag.
