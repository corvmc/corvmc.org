...
| [development-agreements-spec.md](specs/development-agreements-spec.md) | 📋     | spec      | Sponsors and grants as two staff-only modules (amended after #1479 was reversed): `sponsor`/`sponsorship` and `funder`/`grant_application`/`grant_report`, a shared ISO-day deadline helper, `sponsor.*` and `grant.*` capabilities. Placement, reminders and renewals are sub-issues (#595, #574)                                                                                                         |
