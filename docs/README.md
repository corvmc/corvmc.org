...
| [production-projects-spec.md](specs/production-projects-spec.md)           | 🔧     | spec      | Every production is a project, specialised: `project.kind`, `project_committee` so Booking and Production both take part in a show, `production.book` / `production.run` guards, crew shifts on the show's clock, show money in project burn, and the staff nav's Planning section. Built on `feature/production-projects` (#1673) |
...
| [domain-model.md](architecture/domain-model.md)                                       | ✅     | What the tables _mean_: three verticals over three horizontals, and the shapes that recur                                 |
