...
| [ticket-sliding-scale-spec.md](specs/ticket-sliding-scale-spec.md)         | 🔧     | split     | **Partly shipped** — `event.ticketPriceFloorCents` and `finance/ticket-split.ts` exist; the checkout surface does not. NOTAFLOF online: a suggested price with a per-event floor that defaults to $0, and a split bar dividing what the buyer pays between the acts and the collective. Recorded, not routed — an act is paid like a contractor, never through Connect. Supersedes the member ticket discount |
