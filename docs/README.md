...
| [financial-record-spec.md](specs/financial-record-spec.md)             | 🔧     | split     | Phases 1–2 shipped in #838 — the table, the service, the vocabulary, the seed; the checkout is its only writer so far (#825). Amends the above: Stripe owns settlement, a local `financial_entry` owns accounting — outflow, in-kind and pass-through, which Stripe cannot see |
...
| [reporting-spec.md](specs/reporting-spec.md)                                         | 🔧     | split     | Phases 1 and 3 shipped — the `range`/`bucket`/`csv` kit and the first CSV export. Phase 2 (`DateRangeFilter`) and phase 4 (the annual rollup, #587) are open. Module-owned reports over a shared kit; which vendor answers which question |
