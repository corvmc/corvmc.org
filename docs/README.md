...
| [tap-to-pay-spec.md](specs/tap-to-pay-spec.md)                         | 📋     | spec      | Card-present door sales on one sideloaded Android phone: Stripe Terminal as a third `PaymentGateway` driver, a Capacitor shell pointed at the live site, `terminal_payment` because `payment_cache.userId` is `NOT NULL`. Scopes #612's unattended kiosk out — Tap to Pay cannot be one — and adds no device table |
