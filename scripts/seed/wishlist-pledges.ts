 ...
 * Two are a non-member's (#1565): one confirmed, which holds its entry, and one
 * still pending its email, which holds nothing.
 */
export async function seedWishlistPledges(users: readonly { id: string }[]) {
	...
	const rows = await batchInsert(wishlistPledge, [
		...
		// Only on a second gear request, so no entry carries two live pledges.
		...(gear[1]
			? [
					{
						userId: null,
						guestName: 'Robin Tern',
						guestEmail: 'robin.tern@example.com',
		...
		{
			userId: null,
			guestName: 'Sam Pending',
			guestEmail: 'sam.pending@example.com',
			confirmTokenHash: 'seed-unconfirmed',
			subjectType: 'item',
			subjectId: supplies[1]?.id ?? supplies[0].id,
			status: 'pending',
